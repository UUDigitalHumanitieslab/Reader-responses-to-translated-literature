...
WINDOW_SIZE = 4
INPUT_FACTORS = ['id', 'original_language', 'edition_language', 'book_title', 'language',
                 'age_category', 'book_genre', 'rating_no', 'is_translated', 'mention_count']
OUTPUT_FILE = 'reviews_PHN.csv'
    ...
    for lang in LANGUAGES:
        ...
        for i, row in df.iterrows():
            ...
            if cats[0][1] >= 2 and isinstance(cats[0][0], str):
                output_list.append(
                    {'word': row['word'], 'language': lang, 'category': cats[0][0]})
...
def count_sentiments(reviews_file):
    ...
    if isfile(OUTPUT_FILE):
        # remove earlier file, since we write out in append mode
        remove(OUTPUT_FILE)
    write_header = True
    for i, row in reviews.iterrows():
        if row['mentions_translation'] and row['language'] in LANGUAGES:
            ...
            relevant_sentiments = sentiments[sentiments['language']
                                             == row['language']]
            for k, word in enumerate(words):
                if re.search(pattern, word):
                    relevant_words = [words[j] for j in range(
                        k - WINDOW_SIZE, k + WINDOW_SIZE + 1) if 0 <= j < len(words)]
                    for m, sen in relevant_sentiments.iterrows():
                        if sen['word'] in relevant_words:
                            data[sen['category']] += 1
            output = pd.DataFrame(data, index=[i])
            output.to_csv(OUTPUT_FILE, mode='a', header=write_header)
            write_header = False
